...
Future<List<dynamic>> getSuggestions(String query) async {
  ...
  try {
    final url = Uri.parse('$_geocodingUrl?name=$query&count=5&language=es');
...
- `Uri.parse(...)` → Construye la URL completa de la petición. `?name=$query&count=5&language=es` son los parámetros: busca por nombre, devuelve máximo 5 resultados, en español.
...
Cuando el usuario usa el GPS, obtenemos coordenadas pero no un nombre. Esta función las convierte en algo legible como "Málaga, Andalucía, España".
